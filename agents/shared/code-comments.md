...
- Never write change-log comments (`// changed from X to Y`, `// updated for feature Z`). That explanation belongs in the commit message, which is where this repository puts the detail: what changed and why.
- Keep existing comments that explain logic, behaviour, or intent, even if they seem obvious. Only remove comments that are outdated or factually wrong.
