...
2. **バイアス排除読み**: 指示を「白紙」の実行者に読ませる。Agent tool（Codex ではネイティブ subagent）で **新規 subagent を dispatch** する。自己再読で済ませない（直前に書いた文章を客観視することは構造的に不可能）。並列で複数シナリオを同時実行する場合は単一メッセージ内で複数 Agent 呼び出しを並べる。dispatch 不能環境の扱いは「環境制約」節を参照。
...
何を測り、出た数字をどう読むかは [`references/metrics.md`](references/metrics.md)。
数字が想定と食い違ったとき、`tool_uses` の偏りを見るとき、修正の効果を見積もるときに読む。
...
新規 subagent を dispatch できない環境（既に subagent として動作している、Agent tool やネイティブ subagent が使えない等）では、本 skill は **適用しない**。
...
打ち切りたくなったとき、結果が動かないときは [`references/pitfalls.md`](references/pitfalls.md) を読む。
出てくる合理化のほぼ全部がそこに列挙してある。
