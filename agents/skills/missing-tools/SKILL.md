...
Never install tools globally: not `npm install -g`, `npm i -g`, `pnpm add -g`, `yarn global add`, `bun add -g`, `uv tool install`, `brew install`, or any language-specific global installer. This machine is declared in Nix, and a global install is invisible to that declaration and missing on the next machine.
