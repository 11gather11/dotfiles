...
get_dotfiles_dir() {
  ...
  # Go up two directories from bin/lib/common.sh to get dotfiles root
  cd "$script_dir/../.." &>/dev/null && pwd
}

# Set DOTFILES directory as readonly (only if not already set)
if [[ -z "${DOTFILES:-}" ]]; then
...
setup_colors() {
  ...
  # Standard colors
  set_color RED '\033[0;31m'
  set_color GREEN '\033[0;32m'
  set_color YELLOW '\033[0;33m'
  set_color BLUE '\033[0;34m'
  set_color MAGENTA '\033[0;35m'
  set_color CYAN '\033[0;36m'
  set_color BLACK '\033[0;30m'
  ...
  # Background colors
  set_color BG_RED '\033[41m'
  set_color BG_GREEN '\033[42m'
  set_color BG_YELLOW '\033[43m'
