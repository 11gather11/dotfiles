...
in
{
  ...
  home.activation.installGitHooks = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if git diff HEAD^..HEAD --name-only 2>/dev/null | grep -qE '^(flake\\.nix|flake\\.lock|nix/|agents/|codex/AGENTS\\.md)'; then
    ...
    if git diff HEAD@{1}..HEAD --name-only 2>/dev/null | grep -qE '^(flake\\.nix|flake\\.lock|nix/|agents/|codex/AGENTS\\.md)'; then
      echo "Nix configuration changed. Applying changes..."
      nix run .#switch
    fi
    HOOK_EOF
    ...
    if git diff ORIG_HEAD..HEAD --name-only 2>/dev/null | grep -qE '^(flake\\.nix|flake\\.lock|nix/|agents/|codex/AGENTS\\.md)'; then
